...
(function () {
  ...
  const catalog = [
    ...
    item('avatar-rashid', 'avatar', 'راشد التكتيكي', 720, 'legendary', 9, '♟', { colors: ['#67e8f9', '#172554'], interactive: true, personality: { id: 'tactician', title: 'المخطط الهادئ', greeting: 'نقرأ الجولة قبل أن تبدأ.', correct: 'حركة محسوبة يا بطل.', wrong: 'معلومة جديدة، والخطة مستمرة.', win: 'الخطة اكتملت بإتقان!', streak: 'خطوة بخطوة نحصد الجولات.', timeout: 'لا داعي للعجلة، فكّر جيدًا.', voice: { pitch: .88, rate: .92, gender: 'male' } } }),
    item('avatar-lama', 'avatar', 'لمى البرق', 760, 'legendary', 10, 'ϟ', { colors: ['#fde047', '#db2777'], interactive: true, personality: { id: 'spark', title: 'سريعة البديهة', greeting: 'جاهز؟ نخطف الجولة!', correct: 'برق! هذه هي الإجابة.', wrong: 'ولا يهمك، التالية لنا.', win: 'انتصار بسرعة البرق!', streak: 'سلسلة برق ولا أروع!', timeout: 'الوقت يجري… احسم بسرعة.', voice: { pitch: 1.15, rate: 1.12, gender: 'female' } } }),
    item('avatar-shaheen', 'avatar', 'شاهين الحارس', 820, 'legendary', 12, '◈', { colors: ['#fb923c', '#0f172a'], interactive: true, personality: { id: 'guardian', title: 'ثابت تحت الضغط', greeting: 'ثبات وتركيز حتى النهاية.', correct: 'ممتاز، حافظ على هذا النسق.', wrong: 'اهدأ… ما زالت المباراة بيدك.', win: 'حُسمت بثبات الأبطال.', streak: 'دفاع صلب وتقدّم ثابت.', timeout: 'خذ نفسًا، ما زال الوقت يكفي.', voice: { pitch: .9, rate: .9, gender: 'male' } } }),
    item('avatar-noura', 'avatar', 'نورا النخبة', 880, 'legendary', 14, '✧', { colors: ['#c4b5fd', '#0e7490'], interactive: true, personality: { id: 'oracle', title: 'قارئة الاحتمالات', greeting: 'كل احتمال يقودنا للفوز.', correct: 'توقّع دقيق وقرار أجمل.', wrong: 'غيّر الزاوية وستظهر الإجابة.', win: 'كما توقعت… فوز مستحق!', streak: 'كأنك تقرأ الأسئلة قبل ظهورها!', timeout: 'استشعري الإجابة… الوقت يقترب.', voice: { pitch: 1.05, rate: .95, gender: 'female' } } }),
    item('avatar-bandar', 'avatar', 'بندر السريع', 340, 'rare', 4, '➳', { colors: ['#38bdf8', '#0f172a'], interactive: true, personality: { id: 'sprinter', title: 'يركض نحو كل إجابة', greeting: 'جاهز للانطلاق من الصفارة!', correct: 'سبقتهم بالكامل!', wrong: 'زلة بسيطة، نلحق الجولة الجاية.', win: 'وصلنا الخط قبل الجميع!', streak: 'سرعة وثبات… سلسلة ما تنكسر.', timeout: 'ثوانٍ تفصلنا، يلا نحسمها.', voice: { pitch: 1.08, rate: 1.15, gender: 'male' } } }),
    item('avatar-reem', 'avatar', 'ريم الذكية', 380, 'rare', 5, '❉', { colors: ['#f472b6', '#312e81'], interactive: true, personality: { id: 'clever', title: 'ترى الحل من زاوية غريبة', greeting: 'هاتوا أصعب سؤال عندكم.', correct: 'بالضبط كذا قلت لكم!', wrong: 'حيلة لطيفة، بس ما نفعت هالمرة.', win: 'الذكاء انتصر أخيرًا.', streak: 'فكرة وراء فكرة… ما تنقطع.', timeout: 'خلوني أفكر بصوت عالٍ.', voice: { pitch: 1.12, rate: 1.02, gender: 'female' } } }),
    item('avatar-joud', 'avatar', 'جود الحالمة', 480, 'epic', 7, '❋', { colors: ['#a5b4fc', '#1e1b4b'], interactive: true, personality: { id: 'dreamer', title: 'تحلم بالفوز وتخطط له', greeting: 'كل سؤال طريق نحو الحلم.', correct: 'كأن الإجابة كانت تنتظرك.', wrong: 'ولا يهمك، الحلم ما زال قريب.', win: 'الحلم تحقق أمام أعيننا.', streak: 'تتوالى الإجابات كسحابة أحلام.', timeout: 'خذ لحظة… الوقت يمهلك قليلًا.', voice: { pitch: 1.02, rate: .85, gender: 'female' } } }),
    item('avatar-faris', 'avatar', 'فارس الميدان', 520, 'epic', 8, '⚑', { colors: ['#f97316', '#7c2d12'], interactive: true, personality: { id: 'warrior', title: 'يخوض كل جولة كأنها الأخيرة', greeting: 'الميدان لي، تفضلوا.', correct: 'ضربة موفقة يا أبطال!', wrong: 'جولة خاسرة، بس الحرب طويلة.', win: 'انتصار يليق بفارس!', streak: 'سلسلة انتصارات متتالية!', timeout: 'الوقت يضغط… اثبت الآن.', voice: { pitch: .92, rate: 1.06, gender: 'male' } } }),
    item('avatar-khaled', 'avatar', 'خالد الأسطورة', 860, 'legendary', 13, '♔', { colors: ['#facc15', '#1e293b'], interactive: true, personality: { id: 'legend', title: 'خبرة سنوات في كل إجابة', greeting: 'شرّفتوني بمنافستي.', correct: 'هذا مستوى الأساطير.', wrong: 'حتى الأساطير تخطئ أحيانًا.', win: 'أسطورة أخرى تُكتب اليوم.', streak: 'سلسلة تليق باسمي.', timeout: 'الحكمة تحتاج لحظة صمت.', voice: { pitch: .8, rate: .88, gender: 'male' } } }),
    item('avatar-leen', 'avatar', 'لين النجمة', 900, 'legendary', 15, '☆', { colors: ['#fde68a', '#db2777'], interactive: true, personality: { id: 'star', title: 'تضيء كل جولة بحضورها', greeting: 'أنا هنا، فلتبدأ الإضاءة!', correct: 'تألقتِ يا بطلة!', wrong: 'حتى النجوم تخفت أحيانًا.', win: 'نجمة الجولة بلا منازع!', streak: 'توهج مستمر… سلسلة من نور.', timeout: 'لحظة تركيز قبل التألق.', voice: { pitch: 1.18, rate: 1.08, gender: 'female' } } }),
