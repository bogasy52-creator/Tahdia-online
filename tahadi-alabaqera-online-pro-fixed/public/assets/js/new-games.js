(function () {
  ...
  const aiGain = (correct = true) => window.TAHADI_BOT?.arcadeScore?.(game, { correct }) ?? Math.round(100 + Math.random() * 60);
  ...
  function finish(note) {
    ...
    result.innerHTML = `<div class="arcade-badge">${win ? '🏆 فوز مستحق' : '💪 مباراة قوية'}</div><div class="big">${Math.round(state.score)}</div><p class="arcade-note">${esc(note)}</p><div class="arcade-hud"><div class="arcade-stat"><small>نتيجتك</small><b>${Math.round(state.score)}</b></div><div class="arcade-timer">VS</div><div class="arcade-stat"><small>خصم BOT</small><b>${Math.round(state.aiScore)}</b></div></div><div class="award-list">${(p.unlocked || []).slice(-4).map((t) => `<span class="award-pill">🏅 ${esc(t)}</span>`).join('')}</div><p class="arcade-note">+${p.deltaXp || 10} XP • +${p.deltaCoins || 3} عملة</p><div class="arcade-actions-row"><button class="bs-btn primary" id="newAgain">إعادة التحدي</button><a class="bs-btn" href="/matchmaking">👥 لاعب حقيقي</a><a class="bs-btn" href="/">الرئيسية</a></div>`;
