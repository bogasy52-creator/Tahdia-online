(() => {
  ...
  function play(name,opts={}){if(settings.muted||!settings.effects)return;unlock();const a=audio[name];if(a){try{a.pause();a.currentTime=0;a.volume=Math.min(1,(opts.volume??.9)*settings.volume);a.play().catch(()=>{})}catch{};return}const map={click:[520,.04,.025],move:[660,.045,.025],dice:[150,.12,.06,'triangle'],card:[420,.06,.035],ladder:[590,.15,.05],snake:[170,.22,.05,'sawtooth'],buzzer:[880,.16,.08,'square'],win:[784,.18,.07],error:[180,.15,.05]};const v=map[name];if(v){tone(...v);if(name==='win'){tone(988,.2,.065,'sine',.13);tone(1175,.28,.06,'sine',.27)}}}
